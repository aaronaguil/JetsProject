...
public class JetTest {
	...
	public static void printMenu(Scanner scanner) {
		...
		case "1":
			listFleet(scanner);
			break;
		case "2": fastestJet(scanner);
			break;
		case "3": longestRange(scanner);
			break;
		case "4": addJet(scanner);
			break;
		case "5":
			break;
		default:
	...
	public static void listFleet(Scanner scanner) {
		for (Jet j : jets) {
			...
			System.out.println("Pilot\tName: " + j.getPilot().getFirstName() + " " + j.getPilot().getLastName());
			System.out.println("\tRame: " + j.getPilot().getRank());
		}
	...
	public static void fastestJet(Scanner scanner) {
		...
		Jet fastest = jets[0];
		for (Jet j : jets) {
			//if speed of current jet is less than the next, store the next element as the fastest jet
		...
		System.out
				.println("Pilot\tName: " + fastest.getPilot().getFirstName() + " " + fastest.getPilot().getLastName());
		System.out.println("\tRame: " + fastest.getPilot().getRank());
		System.out.println("***************************");
		printMenu(scanner);
	}
	...
	public static void longestRange(Scanner scanner) {
		...
		for (Jet j : jets) {
			//if range of current jet is less than the next, store the next element as the fastest jet
		...
		System.out.println(
				"Pilot\tName: " + longestRange.getPilot().getFirstName() + " " + longestRange.getPilot().getLastName());
		System.out.println("\tRame: " + longestRange.getPilot().getRank());
		System.out.println("***************************");
		printMenu(scanner);
	}
	...
	public static void addJet(Scanner scanner){
		//create new array that is one element bigger than the previous array of jets
		...
		for (int i = 0; i<pilots.length; i++) {
			System.out.println("[" + (i+1) + "] " + pilots[i].getFirstName() + " " + pilots[i].getLastName() + "(" + pilots[i].getRank() + ")");
		}
